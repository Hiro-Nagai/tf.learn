...
* インフラエンジニアでもコード書く必要性を実感。細かい文法エラーに引っかかった。
...
- 隠しファイルに注意。
- terraform initは.tfファイルがあるところで一回のみ実行。（理由:.terraform自動生成につき、何度も実行すると重いファイルが生成しまくるため）
- Outputブロックは、resouceブロックの結果がどう出て欲しいか名前付けするとこ
- パブリックIPはEC2起動ごとに変わる
